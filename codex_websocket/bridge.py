...
class CodexBridge:
    ...
    async def _connect_and_initialize(self) -> None:
        import websockets
        url = self._injected_ws_url or f"ws://127.0.0.1:{self.port}"
    ...
    def get_status(self) -> Result:
        """Return bridge status: connection state, active tasks, total threads, model."""
        from websockets.protocol import State as WsState

        connected = self.ws is not None and self.ws.state == WsState.OPEN
        active_tasks = len(self._task_map)

        # Query the server for the real thread count instead of using the
        # local _threads dict (which always equals active_tasks since the two
        # dicts are modified in lockstep).
